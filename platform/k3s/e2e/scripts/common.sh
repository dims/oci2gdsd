...
QWEN_GDS_RUNTIME_DOCKERFILE="${QWEN_GDS_RUNTIME_DOCKERFILE:-${REPO_ROOT}/platform/k3s/workloads/pytorch/Dockerfile.vllm-runtime-gds}"
...
if [[ -z "${QWEN_HELLO_TEMPLATE}" ]]; then
  QWEN_HELLO_TEMPLATE="${REPO_ROOT}/platform/k3s/workloads/pytorch/qwen-k3s-hello-deployment.yaml.tpl"
fi
...
PYTORCH_DAEMON_NATIVE_CPP="${PYTORCH_DAEMON_NATIVE_CPP:-${REPO_ROOT}/platform/k3s/workloads/pytorch/native/oci2gds_torch_native.cpp}"
validate_workload_runtime
configure_workload_runtime
validate_deploy_assets
enforce_strict_gds_policy
...
validate_qwen_hello_example() {
  ...
  local app_dir="${REPO_ROOT}/platform/k3s/workloads/pytorch/app"
  local native_dir="${REPO_ROOT}/platform/k3s/workloads/pytorch/native"
  if [[ ! -f "${template}" ]]; then
