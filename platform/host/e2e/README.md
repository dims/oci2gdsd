...
- Probe compiles native extension from shared source file: `platform/k3s/workloads/pytorch/qwen-hello/native/oci2gds_torch_native.cpp`.

Runtime dependency behavior:

- `platform/k3s/workloads/pytorch/qwen-hello/app/deps_bootstrap.py` is check-only by default and fails fast when required Python packages are missing.
