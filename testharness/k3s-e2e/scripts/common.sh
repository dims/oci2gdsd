...
QWEN_GDS_RUNTIME_DOCKERFILE="${QWEN_GDS_RUNTIME_DOCKERFILE:-${REPO_ROOT}/examples/k3s/qwen-hello/Dockerfile.vllm-runtime-gds}"
...
if [[ -z "${QWEN_HELLO_TEMPLATE}" ]]; then
  QWEN_HELLO_TEMPLATE="${REPO_ROOT}/examples/k3s/qwen-hello/qwen-k3s-hello-deployment.yaml.tpl"
fi
OCI2GDSD_DAEMON_TEMPLATE="${OCI2GDSD_DAEMON_TEMPLATE:-${REPO_ROOT}/examples/k3s/daemonset/oci2gdsd-daemonset.yaml.tpl}"
PYTORCH_DAEMON_CLIENT_TEMPLATE="${PYTORCH_DAEMON_CLIENT_TEMPLATE:-${REPO_ROOT}/examples/k3s/daemonset/pytorch-daemon-client-job.yaml.tpl}"
PYTORCH_DAEMON_CLIENT_SCRIPT="${PYTORCH_DAEMON_CLIENT_SCRIPT:-${REPO_ROOT}/examples/k3s/daemonset/pytorch_daemon_client.py}"
TENSORRT_DAEMON_CLIENT_TEMPLATE="${TENSORRT_DAEMON_CLIENT_TEMPLATE:-${REPO_ROOT}/examples/k3s/daemonset/tensorrt-daemon-client-job.yaml.tpl}"
TENSORRT_DAEMON_CLIENT_SCRIPT="${TENSORRT_DAEMON_CLIENT_SCRIPT:-${REPO_ROOT}/examples/k3s/daemonset/tensorrt_daemon_client.py}"
VLLM_DAEMON_CLIENT_TEMPLATE="${VLLM_DAEMON_CLIENT_TEMPLATE:-${REPO_ROOT}/examples/k3s/daemonset/vllm-daemon-client-job.yaml.tpl}"
VLLM_DAEMON_CLIENT_SCRIPT="${VLLM_DAEMON_CLIENT_SCRIPT:-${REPO_ROOT}/examples/k3s/daemonset/vllm_daemon_client.py}"
PYTORCH_DAEMON_NATIVE_CPP="${PYTORCH_DAEMON_NATIVE_CPP:-${REPO_ROOT}/examples/k3s/qwen-hello/native/oci2gds_torch_native.cpp}"
validate_workload_runtime
configure_workload_runtime
validate_deploy_assets
enforce_strict_gds_policy
...
validate_qwen_hello_example() {
  ...
  local app_dir="${REPO_ROOT}/examples/k3s/qwen-hello/app"
  local native_dir="${REPO_ROOT}/examples/k3s/qwen-hello/native"
  if [[ ! -f "${template}" ]]; then
