...
resolve_model_identity() {
  if [[ -n "${MODEL_REF_OVERRIDE}" && -n "${MODEL_DIGEST_OVERRIDE}" ]]; then
    ...
    die "missing model identity; set MODEL_REF_OVERRIDE and MODEL_DIGEST_OVERRIDE, or run make k3s-e2e once to generate ${WORK_DIR}/packager/output/manifest-descriptor.json.
Example:
